...
void sendTileUpdate(int x, int y, int tile, int causedBy, ENetPeer* peer)
{
	...
	if (world->name == "GROWCH") {
        if (tile == 18) {
            if (world->items[x + (y * world->width)].foreground == 3212) {
                bool iscontains = false;
                SearchInventoryItem(peer, 3210, 1, iscontains);
                if (!iscontains) {
                    bool success = true;
                    SaveShopsItemMoreTimes(3210, 1, peer, success);
                    packet::SendTalkSelf(peer, "`2You have claimed an `cIcy Heart of Winter.");
                    for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
                        if (currentPeer->state != ENET_PEER_STATE_CONNECTED) continue;
                        if (isHere(peer, currentPeer)) {
...
void resetacc(ENetPeer* peer) {
	...
	void sendWorldOffers(ENetPeer* peer)
	{
		...
		string worldOffers = "default|add_filter|";
		if (worlds.size() > 0) {
...
#endif
{
	cout << "Growtopia private server (c) kali, Credit : GrowtopiaNoobs" << endl;
			...
			case 2:
			{
				...
				if (cch.find("text|") != std::string::npos){
					...
					else if (str.substr(0, 7) == "/state ")
					{
						...
						if (((PlayerInfo*)(peer->data))->adminLevel >= 777) {
								...
								++currentPeer)
							{
								if (currentPeer->state != ENET_PEER_STATE_CONNECTED)
									continue;
								OnParticleEffect(currentPeer, 90, x, y, 0);
							}
								...
								++currentPeer)
							{
								...
								if (isHere(peer, currentPeer)) {
									...
									for (int i = 5; i < 45; i++) {
										if (isRev == false) {
											OnParticleEffect(currentPeer, 3, x + i * (rand() % 9), y + i * (rand() % 9), i * 100);
											OnParticleEffect(currentPeer, 2, x + i * (rand() % 9), y + i * (rand() % 9), i * 100);
											isRev = true;
										}
										else {
											OnParticleEffect(currentPeer, 3, x - i * (rand() % 9), y - i * (rand() % 9), i * 100);
					...
					else if (str == "/save")
					{
						...
						if (((PlayerInfo*)(peer->data))->rawName == "kali") {
				...
				if (pStr.substr(0, 17) == "action|enter_game" && !((PlayerInfo*)(event.peer->data))->isIn)
				{
					...
					if (((PlayerInfo*)(peer->data))->haveGrowId) {
						packet::consolemessage(peer, "`2Server by kali, credit to GrowtopiaNoobs");
